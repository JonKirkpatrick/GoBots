...
if ! command -v docker >/dev/null 2>&1; then
  echo "[$(date -Iseconds)] update failed: docker is not installed" >>"$LOG_FILE"
  exit 1
fi

if ! docker compose version >/dev/null 2>&1; then
  echo "[$(date -Iseconds)] update failed: docker compose plugin not available" >>"$LOG_FILE"
  exit 1
fi
...
cd "$DEPLOY_DIR"
docker compose --env-file .env up -d --build >>"$LOG_FILE" 2>&1
